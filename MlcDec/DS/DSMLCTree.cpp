...
DSMLCTree::DSMLCTree(uint dim_, uint n_, bool is_isolate) : root(nullptr), dim(dim_), n(n_), num_of_nodes(0),
                                                            isolate(is_isolate) {
    i_off = dim_ * sizeof(uint);
    d_off = i_off + sizeof(uint);
    c_off = d_off + sizeof(struct DS_Diff);
}
...
uint DSMLCTree::Search(const vector<uint> &k_vec, uint *core, uint &length) {
...
uint DSMLCTree::Retrieve(Node *node, uint *core) const {
    uint length = 0;
    DS_Diff *diff;
...
uint DSMLCTree::PSearch(const vector<uint> &k_vec, uint *core, uint &length) {
    uint i, num_of_visits;
    Node *node;
...
uint DSMLCTree::PRetrieve(Node *node, uint *core) const {
    uint length;
#pragma omp parallel
    {
#pragma omp master
...
uint DSMLCTree::Fd(Node *node, uint *core) const {
...
void DSMLCTree::Flush(const string &file) {
...
void DSMLCTree::Flush(std::basic_ofstream<char> &f) {
...
void DSMLCTree::FlushNode(Node *node, unordered_map<Node *, ll_uint> &node2id, std::basic_ofstream<char> &f) const {
    Node *child;
    ll_uint child_id;
    DS_Diff *difference;
    ...
    if (difference->num) {
        f.write(reinterpret_cast<char *> (difference->n_edge), long(dim * sizeof(uint)));
...
DSMLCTree *DSMLCTree::Load(const string &file) {
    uint dimension, n_;
    ...
    auto *tree = new DSMLCTree(dimension, n_);
    tree->Load(f);
    f.close();
    return tree;
}

void DSMLCTree::Load(std::basic_ifstream<char> &f) {
...
void DSMLCTree::LoadNode(Node *node, unordered_map<ll_uint, Node *> &id2node, std::basic_ifstream<char> &f) {
    Node *child;
    ll_uint child_id;
    DS_Diff *difference;
    ...
    if (difference->num) {
        difference->n_edge = difference_buf.Allocate(dim + difference->num);
        difference->vtx_ptr = difference->n_edge + dim;

        f.read(reinterpret_cast<char *> (difference->n_edge), long(dim * sizeof(uint)));
...
bool DSMLCTree::Equals(DSMLCTree &mlc_t) {
...
bool DSMLCTree::IsEqual(Node *node, Node *mlc_tree_node) {
    DS_Diff *d, *mlc_d;
...
void DSMLCTree::MergeBuf(DSMLCTree *mlc_t) {
