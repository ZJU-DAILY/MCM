...
#ifndef MLCDEC_DSMLCTREE_H
#define MLCDEC_DSMLCTREE_H
...
class DSMLCTree {
public:
    explicit DSMLCTree(uint dim_, uint n_, bool is_isolate = true);
    ~DSMLCTree();
    ...
    inline DS_Diff *GetDiff(Node *node) const {
        return reinterpret_cast<DS_Diff *>(reinterpret_cast<char *> (node) + d_off);
    }
    ...
    inline uint *GetDiffEC(Node *node) const {
        return reinterpret_cast<DS_Diff *>(reinterpret_cast<char *> (node) + d_off)->n_edge;
    }

    inline void SetDiff(Node *node, uint length, uint *vtx, const uint * relabel = nullptr) {
        DS_Diff *d = GetDiff(node);
        d->num = length;
        if (length) {
            d->n_edge =  difference_buf.Allocate(dim + length);
            d->vtx_ptr = d->n_edge + dim;
    ...
    static DSMLCTree *Load(const string &file);
    void Load(std::basic_ifstream<char> &f);

    bool Equals(DSMLCTree &mlc_t);
    void MergeBuf(DSMLCTree *mlc_t);
